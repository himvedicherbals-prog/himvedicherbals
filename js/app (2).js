...
(async function init() {
    ...
    renderHeroTrustBadges();  // ✅ FIXED: Now shows dynamic product counts
    renderTestimonials();
    renderFooterLinks();
...
/**
 * ========================================================================
 * ✅ FIXED: Renders hero trust badges with DYNAMIC product counts
 * ========================================================================
 * Previously: Hardcoded static strings ("5+ varieties", "Natural", etc.)
 * Now: Dynamically reads actual product counts from AppData
 */
function renderHeroTrustBadges() {
    const container = document.getElementById('heroTrustBadges');
    if (!container) return;

    // ✅ DYNAMIC CONFIG - maps category slugs to display settings
    const badgeConfig = [
        { emoji: '🌱', slug: 'herbs', unit: 'varieties' },
        { emoji: '🌲', slug: 'resin', unit: 'types' },
        { emoji: '🫒', slug: 'oils', unit: 'oils' },
        { emoji: '🫘', slug: 'seeds', unit: 'kinds' },
        { emoji: '🧼', slug: 'soap', unit: 'options' },
        { emoji: '🧂', slug: 'namak', unit: 'salts' },
        { emoji: '🌶️', slug: 'masala', unit: 'blends' },
        { emoji: '🍄', slug: 'mushroom', unit: 'items' }
    ];

    // ✅ Generate badges with dynamic counts from actual product data
    const badges = badgeConfig.map(b => {
        const count = AppData.getProductsByCategory(b.slug).length;
        const category = AppData.categories.find(c => c.slug === b.slug);
        
        return {
            emoji: b.emoji,
            label: category ? category.label : b.slug,
            sub: `${count}+ ${b.unit}`  // Dynamic count!
        };
    });
